...
function ProgressSideBarNav(props) {
  const stages = [`Select Candidate`, `Select Company`, `Fill Report Details`];
  return (
    <aside className="progress--side-bar">
      <section className="progress--steps">
        {stages.map((el, i) => {
          return <ProgressStep stepName={el} step={props.step} stage={i + 1} />;
        })}
      </section>
      <section className="progress--completed">
        <div className={props.candidate.candidateName ? `` : `step-hidden`}>
        ...
        <div className={props.company.companyName ? `` : `step-hidden`}>
          ...
          <div className="progress--detail">{props.company.companyName}</div>
        </div>
      </section>
    </aside>
  );
